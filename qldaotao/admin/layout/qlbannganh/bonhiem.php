...
<div class="row">
	...
	<div class="col-md-6 text-center">
		...
		<form class="form-inline" action="index.php?page=qlbannganh&act=bonhiem&bn_id=<?php echo $bn_id; ?>" method="post">
...
<form action="index.php?page=qlbannganh&bn_id=<?php echo $bn_id;?>&act=bonhiem" method="POST">
	...
	<div class="col-md-12">
		<table class="table table-striped table-hover table-bordered">
			...
			<tbody>
				...
				$base_url= 'index.php?page=qlbannganh&act=bonhiem&';
				if(isset($_GET['start']))
					$s = $_GET['start'];
				else
					$s = 0;	
				$query = mysqli_query($con, "SELECT * FROM nhan_su WHERE ns_id NOT IN (SELECT ns_id FROM bannganh_nhansu WHERE bn_id=$bn_id AND (bn_ns_quyen='Trưởng ban' OR bn_ns_quyen='Phó ban' OR bn_ns_quyen='Ủy viên'))"); 
				$tongsodong = mysqli_num_rows($query);

				$sql = "SELECT * FROM nhan_su WHERE ns_id NOT IN 
					   (SELECT ns_id FROM bannganh_nhansu bn_ns WHERE bn_ns.bn_id=$bn_id AND (bn_ns_quyen='Trưởng Ban' OR bn_ns_quyen='Phó ban' OR bn_ns_quyen='Ủy viên')) ORDER BY ns_id DESC limit $s,$soproduct";
				//echo $sql;
