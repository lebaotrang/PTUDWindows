...
function checkQuyen($con, $val=array()){ 
    ...
    <div>
    	...
    	while($row=mysqli_fetch_object($qr)){ ?>
    		<input type="checkbox" name="checkQuyen[]" value="<?php echo $row->q_id ?>" <?php if(in_array($row->q_id, $val)) echo "checked"?>><?php echo $row->q_ten; ?>&nbsp;&nbsp;
    		<?php
    	} ?>
    </div>
    <?php
...
//Combobox CTDT
function showDaoTaoList($con, $selectedValue=0) {
	//var_dump($selectedValue);
    $query = "select * from daotao";
    $result = mysqli_query($con, $query);
    echo "<select name='slDaoTao' class='form-control' id='slDaoTao'>
			<option value='0'>Chọn CTDT</option>";
    while ($row = mysqli_fetch_array($result)) {
		if ($row['dt_id'] == $selectedValue) {
			echo "<option value='" . $row['dt_id'] . "' selected>" . $row['dt_ten'] . "</option>";
		} else {
			echo "<option value='" . $row['dt_id'] . "'>" . $row['dt_ten'] . "</option>";
		}
    }
    echo "</select>";
}
...
//combobox hocky
function showHocKy($con, $val="") { ?>
    <select data-placeholder="Chọn học kỳ..." class="form-control" name="slHocKy" id="slHocKy">
        <?php
        for($i=1; $i<=3; $i++){ ?>
            <option value="<?php echo $i; ?>" <?php if($val=="$i") echo "selected";?>><?php echo 'Học kỳ '.$i; ?></option>
            <?php
        } ?>
    </select>
    <?php
...
//combobox nienkhoa
function showNienKhoa($con, $val="") { ?>
    <select data-placeholder="Chọn năm..." class="form-control" name="slNienKhoa" id="slNienKhoa">
        <?php
        for($i=2015; $i<=2025; $i++){ ?>
            <option value="<?php echo $i; ?>" <?php if($val=="$i") echo "selected";?>><?php echo $i."-".($i+1); ?></option>
            <?php
        } ?>
    </select>
    <?php
...
//lay hk hien tai
function HKy_Hientai(){
    $hocky = array(
        1 => array(9, 10, 11, 12, 1),
        2 => array(2, 3, 4, 5),
        3 => array(6, 7, 8)
    );
